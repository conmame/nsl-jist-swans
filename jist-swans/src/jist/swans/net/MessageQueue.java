...
public interface MessageQueue {
	...
	// 2007/11/12 大藪良祐 DropMessageクラスの追加 START
	...
	public class DropMessageQueue implements MessageQueue {
		 ...
		 *            maximum number of items allowed in list
		 */
		public DropMessageQueue(int priorities, int capacity) {
			heads = new QueuedMessage[priorities];
			tails = new QueuedMessage[priorities];
			topPri = heads.length;
			size = 0;
		...
		public void insert(QueuedMessage msg, int pri) {
			if (size == capacity) {
				...
				// 2007/11/12 濱田 浩平 キュー超過時の処理 START
				// キューの最後のパケットを落とす
				System.out.println("list maximum exceeded");
				return;
				// 2007/11/12 濱田 浩平 キュー超過時の処理 END

			}
			size++;
			topPri = Math.min(pri, topPri);
			QueuedMessage tail = tails[pri];
			if (tail == null) {
				heads[pri] = msg;
				tails[pri] = msg;
		...
		// 2007/11/12 大藪良祐 DropMessageクラスの追加 END
