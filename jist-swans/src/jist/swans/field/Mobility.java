...
public interface Mobility
{
  ...
  public static class RandomWaypoint implements Mobility
  {
    ...
    public RandomWaypoint(Location.Location2D bounds, String config)
    {
    	...
    	// 2007/09/17 à_“c _•½ RandomWaypointƒ‚ƒfƒ‹‚ÌŽÀ‘• START
        String[] data  = config.split(",");
        if(data.length!=4)
        {
          //throw new RuntimeException("expected format: fixedradius,randomradius,pausetime(in seconds)");
          // 2008/09/10 à_“c _•½ ƒGƒ‰[ƒƒbƒZ[ƒW‚ÌC³
          throw new RuntimeException("expected format: pauseTime,precision,minspeed,maxspeed");
        }
        long pauseTime = Long.parseLong(data[0])*Constants.SECOND;
        float precision = Float.parseFloat(data[1]);
        float minspeed = Float.parseFloat(data[2]);
        float maxspeed = Float.parseFloat(data[3]);
        init(bounds, pauseTime, precision, minspeed, maxspeed);
        
        // 2007/09/17 à_“c _•½ RandomWaypointƒ‚ƒfƒ‹‚ÌŽÀ‘• END
    }
  ...
  public static class Teleport implements Mobility
  {
    ...
    public Teleport(Location.Location2D bounds, long pauseTime)
    {
      this.bounds = bounds;
      // this.pauseTime = pauseTime;
      // 2008/09/10 —§ŽR ’”V ‘Sƒ‚ƒfƒ‹‚Å’PˆÊ‚ð•b‚É“ˆê
