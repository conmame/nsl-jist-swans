...
// Timestamp: <RouteAodv.java Tue 2004/04/06 11:59:52 barr pompom.cs.cornell.edu>
//
 ...
 * @version $Id: RouteAodv.java,v 1.46 2004/04/06 16:07:50 barr Exp $
 * @since SWANS1.0
 */
...
//2007/11/13 大藪良祐 定数の変更および追加 START
  /* RREQ_TIMEOUT_BASEおよびRREQ_TIMEOUT_PER_TTLは削除 */
  public static final int MAX_RREQ_BUFFER_SIZE = 256;
  public static final long AODV_TIMEOUT = 1 * Constants.SECOND;
  public static final long HELLO_INTERVAL = 1 * Constants.SECOND;
  public static final long HELLO_ALLOWED_LOSS = 2;
  
  /** Constant term of the RREQ Timeout base. */
  public static final long NODE_TRAVERSAL_TIME = 40 * Constants.MILLI_SECOND;

  /** Constant term for determine neighbor is active or not */
  public static final long NEXT_HOP_WAIT = NODE_TRAVERSAL_TIME * 2;

  
//2007/11/13 大藪良祐 定数の変更および追加 END
...
//    2007/11/13 大藪良祐 自身のエントリのアップデート方法変更
      thisNode.routeTable.add(thisNode.netAddr, new RouteTableEntry(MacAddress.NULL, this.thisNode.seqNum, 0));
...
//  2007/11/13 大藪良祐 RREQユニキャスト送信メソッド追加 START
    /**
     * Creates and unicasts a RREQ message.
     * For using to determine neighbor is active or not
     * 
     * Also, updates RREQ buffer and routing table.
     */
    public void unicast(MacAddress dest)
    {
      // determine destination sequence number to use, by checking routing table entry
      int destSeqNum=0;
      boolean unknownDestSeqNum=false;
      ...
      // unicast the IP message
      printlnDebug("Unicasting RREQ message with rreqId="+rreqId+", ttl="+ttl, thisNode.netAddr);
      thisNode.self.sendIpMsg(rreqMsgIp, dest);
...
//  2007/11/13 大藪良祐 RREQユニキャスト送信メソッド追加 END
...
//  2007/11/13 大藪良祐 MACアドレスからIPアドレスを求めるメソッド START
    /**
     * Look up neighbor network address for a given destination mac address.
     * 
     * @param key destination mac address
     * @return neighbor network address for this mac address
     */
    public NetAddress lookup(MacAddress macAdd)
    {
      Iterator itr = table.keySet().iterator();
      while (itr.hasNext())
      {
        NetAddress key = (NetAddress)itr.next();
        RouteTableEntry entry = (RouteTableEntry)table.get(key);
    	if(entry.getNextHop().equals(macAdd) && entry.hopCount == 1)
    	{
    	  return key;
    	}
      }
...
//  2007/11/13 大藪良祐 MACアドレスからIPアドレスを求めるメソッド END
...
//    2007/11/13 大藪良祐 登録時間を取得
      this.lastAddTime = JistAPI.getTime();
    }
...
//  2007/11/13 大藪良祐 登録時間取得メソッド START
    /**
     * Returns time of last added.
     * 
     * @return time of last added
     */
    public long getLastAddTime()
    {
      return lastAddTime;
    }
//  2007/11/13 大藪良祐 登録時間取得メソッド END
...
//  2007/11/13 大藪良祐 ネットアドレスのマップを追加
    /** 隣接ネットアドレスを通る宛先ネットアドレスのマップ */
...
//  2007/11/13 大藪良祐 ネットアドレスマップに登録 START
     ...
     * @param msg message to add to queue
     */
    public void add(NetAddress neighbor, NetAddress dest)
    {
      map.put(neighbor, dest);
    }
    
//  2007/11/13 大藪良祐 ネットアドレスマップに登録 END
    
//  2007/11/13 大藪良祐 マップからノードを削除し、そのノードを通るメッセージを削除する START
    
    /**
     * Remove map and msg queue.
     * 
     * @param neighbor net address
     */
    public void removeMap(NetAddress neighbor)
    {
      NetAddress dest =(NetAddress)map.get(neighbor);
...
//  2007/11/13 大藪良祐 マップからノードを削除し、そのノードを通るメッセージを削除する END
...
//    	 2007/11/13 大藪良祐 隣接ノードなのか，隣接ノードを経由するのかで処理わけ START
        NetAddress newDest = (NetAddress)map.get(dest);
        if(newDest==null){
          for (int i=0; i<list.size(); i++)
          {
            if (((NetMessage.Ip)list.get(i)).getDst().equals(dest))
      	...
      	for (int i=0; i<list.size(); i++)
          {
         	  NetAddress msgDest =((NetMessage.Ip)list.get(i)).getDst();
            if (msgDest.equals(dest) || msgDest.equals(newDest))
            {
              NetMessage.Ip msg = (NetMessage.Ip)list.remove(i);
              printlnDebug("Routing IP message to "+nextHop, thisNode.netAddr);
              thisNode.self.sendIpMsg(msg, nextHop);
...
//   	 2007/11/13 大藪良祐 隣接ノードなのか，隣接ノードを経由するのかで処理わけ END
...
//      }
		...
		// 2007/11/13 大藪良祐 RERRをブロードキャスト送信 START
      
      //send the IP message to each precursor
      printlnDebug("Sending RERR to neighbors", thisNode.netAddr);
      thisNode.self.sendIpMsg(ipMsg, MacAddress.ANY);
      if (thisNode.stats != null)
      {
        thisNode.stats.send.rerrPackets++;
        thisNode.stats.send.aodvPackets++;
		...
		// 2007/11/13 大藪良祐 RERRをブロードキャスト送信 END
  ...
  private static class OutgoingSet
  {
	...
	// 2007/11/13 大藪良祐 remove(MacAddress)の追加 START
    /**
     * Removes an item from the outgoing set.
     * 
     * @param m Mac address of the node to remove from set
     */
    public void remove(MacAddress m)
    {
      printlnDebug("Removing "+m+" from precursor set", localAddr);
      map.remove(m);
    }

    // 2007/11/13 大藪良祐 remove(MacAddress)の追加 END
  ...
  private static class OutgoingInfo
  {
    ...
    public void incHelloWaitCount()
    {
  	...
  	// 2007/11/13 大藪良祐 最終送信時間の取得 START
    /**
     * Returns the time that the last message was sent to this precursor.
     * 
     * @return time that last message was sent to precursor
  ...
  // 2007/11/13 大藪良祐 最終ブロードキャスト送信時間を示す変数追加
  /** the time whitch node broadcasted(RREQ,hello). */
  ...
  // 2007/11/13 大藪良祐 RREQユニキャスト送信用タイムアウトメソッド START
  /**
   * This event is called periodically after a route request is originated, until
   * a route has been found.
   * 
   ...
   * @param rreqObj RouteRequest object
   */
  public void RREQUnicastTimeout(Object rreqObj, NetMessage msg, MacAddress nextHopMacAddr)
  {
	RouteRequest rreq = (RouteRequest)rreqObj;
    if(rreq.routeFound){
      //System.out.println("kigengai ok in "+netAddr);
    }else{
        //それでも隣接の反応がない場合、RTからルートを消し、RERR送信後データを再送
        //remove all affected routes in routing table
        this.routeTable.removeNextHop(nextHopMacAddr);
        //remove from precursor set, if it exists
        precursorSet.remove(nextHopMacAddr);
        //remove from outgoing set
        printlnDebug("Removing "+nextHopMacAddr+" from outgoing set");
        outgoingSet.remove(nextHopMacAddr);
        //send RERR message to precursors
        precursorSet.sendRERR(routeTable.destsViaHop(nextHopMacAddr), Constants.TTL_DEFAULT);

        //throw out queued packets
        
        msgQueue.removeMap(rreq.getDest());
        rreqList.remove(rreqObj);
        //データをルート確立して再送
        self.send(msg);
        //System.out.println("kigengai ng in "+netAddr);
    }
  }

  // 2007/11/13 大藪良祐 RREQユニキャスト送信用タイムアウトメソッド END
...
//      helloSendEvent();
		// 2007/11/13 大藪良祐 Helloメッセージのブロードキャスト送信
        broadcastHello();
  ...
  {    
	...
	// 2007/11/13 大藪良祐 Helloメッセージをブロードキャスト送信するメソッド START
  /**
   * Broadcast hello messages to all neighbors past BROADCAST_HELLO_INTERVAL
   * 
   */
  private void broadcastHello()
  {    
    NetMessage.Ip helloMsgIp = null;
    printlnDebug("Broadcasting HELLO message");
    if (JistAPI.getTime() >= lastBroadCastTime + HELLO_INTERVAL)
    {
      if (helloMsgIp == null) 
      {
        HelloMessage helloMsg = new HelloMessage(this.netAddr, this.seqNum);
      ...
      //send hello msg
      self.sendIpMsg(helloMsgIp, MacAddress.ANY);
      if (stats != null)
      {
    	stats.send.helloPackets++;
    	stats.send.aodvPackets++;
      }
      //update time_of_last_broadcast
      lastBroadCastTime = JistAPI.getTime();
    }
  }
//2007/11/13 大藪良祐 Helloメッセージをブロードキャスト送信するメソッド END
  ...
  public void sendIpMsg(NetMessage.Ip ipMsg, MacAddress destMacAddr)
  {
    ...
    if (destMacAddr.equals(MacAddress.ANY))
    {
      ...
      //  2007/11/13 大藪良祐 最終ブロードキャスト送信時間の更新
      //update time_of_last_broadcast
      lastBroadCastTime = JistAPI.getTime();
...
//    }
		...
		// 2007/11/13 大藪良祐 HelloWaitCountのリセット方法変更 START
	    printlnDebug("Peeking at message from "+lastHop+"; resetting outgoing info");
	    //If receive a message from an outgoing link, add info
	    outgoingSet.add(lastHop);
		// 2007/11/13 大藪良祐 HelloWaitCountのリセット方法変更 END
...
//      self.sendIpMsg(ipMsg, nextHopMacAddr);
		...
		// 2007/11/13 大藪良祐 nexthopが期限内かどうかによる処理わけ START
      
      //NextHopが有効か調べる
      long lastMsgTime = outgoingSet.getInfo(nextHopMacAddr).getLastMsgTime();
      if(lastMsgTime + NEXT_HOP_WAIT >= JistAPI.getTime())
      {
    	//Forward message to next hop
        self.sendIpMsg(ipMsg, nextHopMacAddr);
        //System.out.println("yuukou in "+netAddr);
      }else{
    	//期限内でなければ、RREQを隣接にユニキャストしてNEXT_HOP_WAIT間返信がなければ経路確立
        NetAddress neighborAdd = this.routeTable.lookup(nextHopMacAddr);
        if(neighborAdd != null)
        {
    	  // save message in queue
          msgQueue.add(ipMsg);
          msgQueue.add(neighborAdd, ipMsg.getDst());
    	  RouteRequest rreq = new RouteRequest(neighborAdd, this);
          printlnDebug("Adding rreq id "+rreq.getRreqId()+" to rreq list");
          rreqList.add(rreq);
          rreq.unicast(nextHopMacAddr);
          JistAPI.sleep(NEXT_HOP_WAIT);
          self.RREQUnicastTimeout(rreq, msg, nextHopMacAddr);
        }else{
          routeTable.remove(destNetAddr);
          self.send(msg);
        }
      }
		// 2007/11/13 大藪良祐 nexthopが期限内かどうかによる処理わけ END
...
//      receiveHelloMessage((HelloMessage)msg);
		// 2007/11/13 大藪良祐 呼び出す関数の引数の追加
        receiveHelloMessage((HelloMessage)msg, lastHop);
...
//    boolean isDest = rreqMsg.destIp.equals(this.netAddr);
    ...
    // 2007/11/13 大藪良祐 判定処理変更
    boolean isDest = rreqMsg.destIp.equals(this.netAddr) || rreqMsg.destIp.equals(NetAddress.ANY);
...
//      outgoingSet.add(lastHop);
  ...
  private void receiveHelloMessage(HelloMessage helloMsg)
  {
	...
	// 2007/11/13 大藪良祐　receiveHelloMessageの拡張 START
  /**
   * Process an incoming HELLO message.
   * 
   * @param helloMsg incoming hello message
   */
  private void receiveHelloMessage(HelloMessage helloMsg, MacAddress lastHop)
  {
    printlnDebug("Receiving HELLO message from "+helloMsg.getIp());
    
    routeTable.add(helloMsg.getIp(), new RouteTableEntry(lastHop, helloMsg.seqNum, 1));
  }
	// 2007/11/13 大藪良祐　receiveHelloMessageの拡張 END
...
//      selfRoute.setDestSeqNum(this.seqNum);
		// 2007/11/13 大藪良祐 自身の経路情報の更新方法変更
      routeTable.add(this.netAddr, new RouteTableEntry(MacAddress.NULL, this.seqNum, 0));
      routeTable.printTable();
    }
...
//    return RREQ_TIMEOUT_BASE + RREQ_TIMEOUT_PER_TTL * ttl;
		// 2007/11/13 大藪良祐 返却値変更
	  return 2 * NODE_TRAVERSAL_TIME * (2 + ttl);
  }
