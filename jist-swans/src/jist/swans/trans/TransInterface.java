...
public interface TransInterface extends JistAPI.Proxiable, NetInterface.NetHandler
{
  ...
  public static interface TransUdpInterface extends TransInterface
  {
	  ...
	  // 2007/09/17 濱田 浩平 TTLの値を与えて送信 START
	  /**
	   * メッセージの送信(独自改定)
	   * 
	   * @param msg パケットのペイロード部
	   * @param dst 宛先アドレス
	   * @param dstPort 宛先ポート番号
	   * @param srcPort 送信元ポート番号
	   * @param byte priority 優先度
	   * @param ttl TTL
	   */
	  void send(Message msg, NetAddress dst, int dstPort, int srcPort, byte priority,byte ttl);
	  // 2007/09/17 濱田 浩平 TTLの値を与えて送信 END
  }
