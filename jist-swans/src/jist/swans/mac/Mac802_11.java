...
public class Mac802_11 implements MacInterface.Mac802_11 {
	 ...
	 */
	// 2007/09/17 Yutaka Hoshino 正式なRTSThreshold値
	public static final int THRESHOLD_RTS = 2347;
	...
	private short incSeq() {
		...
		// 2007/09/17 濱田 浩平 シーケンスナンバー加算処理の変更 START
		if ((shouldRTS() ? longRetry : shortRetry) == 0) {
			seq = (short) ((seq + 1) % MacMessage.Data.MAX_SEQ);
		}
		// 2007/09/17 濱田 浩平 シーケンスナンバー加算処理の変更 END
		return seq;
	}
	...
	private void sendRts() {
		...
		// System.out.println( this.localAddr+" RTS send at t =
		// "+JistAPI.getTime());
	...
	private void sendCts(MacMessage.Rts rts) {
		// create cts packet
		MacMessage.Cts cts = new MacMessage.Cts(rts.getSrc(), (int) (rts
				.getDuration()
				- MacMessage.Cts.SIZE
				* Constants.SECOND
				/ bandwidth
		...
		// System.out.println( this.localAddr+" CTS send at t =
		// "+JistAPI.getTime());
	...
	private void sendDataBroadcast() {
		...
		// 2007/10/01 濱田 浩平 リトライ値のリセット START
		shortRetry = 0;
		longRetry = 0;
		// 2007/10/01 濱田 浩平 リトライ値のリセット END
	...
	private void receivePacket(MacMessage msg) {
		...
		if (msg == null)
			return;// 2007/09/17 濱田 浩平 null判定追加
	...
	private void receiveCts(MacMessage.Cts cts) {
		if (mode == MAC_MODE_SWFCTS) {
			...
			// 2007/10/08 濱田 浩平 RTSに対するCTS受信時、CWをリセット
			decCW();
	...
	private void receiveAck(MacMessage.Ack ack) {
		if (mode == MAC_MODE_SWFACK) {
			...
			// 2007/10/08 濱田 浩平 状態遷移修正 START
			setBackoff();
			packet = null;
			packetNextHop = null;
			idle();
			netEntity.pump(netId);
			// 2007/10/08 濱田 浩平 状態遷移修正 END
		}
	}
	...
	private void receiveData(MacMessage.Data msg) {
		...
		if (mode == MAC_MODE_SWFDATA || !isAwaitingResponse()) {
			if (MacAddress.ANY.equals(msg.getDst())) {
				...
				// 2007/09/19 Yutaka Hoshino 状態遷移修正 START
				netEntity.receive(msg.getBody(), msg.getSrc(), netId, false);
				idle();
				// 2007/09/19 Yutaka Hoshino 状態遷移修正 END

			} else {
				cancelTimer();
				sendAck(msg);
				// duplicate suppression
				// if(getSeqEntry(msg.getSrc()) < msg.getSeq())
				// 2007/09/17 濱田 浩平 受信処理の厳格化
				// if (!(msg.getRetry() && getSeqEntry(msg.getSrc()) == msg
				// .getSeq())) {
				// 2007/10/08 濱田 浩平 受信判定の修正
				//if (getSeqEntry(msg.getSrc()) < msg.getSeq()) {
				// 2008/01/07 濱田 浩平 受信判定の修正
				if (getSeqEntry(msg.getSrc()) != msg.getSeq()) {
					updateSeqEntry(msg.getSrc(), msg.getSeq());
					netEntity
							.receive(msg.getBody(), msg.getSrc(), netId, false);
				} else {
				}
			}
		}
	...
	private void receiveForeign(MacMessage msg) {
		...
		if (promisc && msg.getType() == MacMessage.TYPE_DATA) {
			...
			// 2008/06/04 濱田　浩平 ACK送信時間待機する
			if(currentTime+msg.getDuration() - JistAPI.getTime() > 0)
				JistAPI.sleep(currentTime+msg.getDuration() - JistAPI.getTime());
	...
	private void radioBusy() {
		...
		case MAC_MODE_SWFDATA:
		case MAC_MODE_SWFACK:
		case MAC_MODE_XCTS: // 2007/09/17 濱田 浩平 RTS/CTS用
		case MAC_MODE_XRTS: // 2007/09/17 濱田 浩平 RTS/CTS用
		case MAC_MODE_XBROADCAST:
		case MAC_MODE_XUNICAST:
		case MAC_MODE_XACK:
			// don't care
	...
	public void timeout(int timerId) {
		...
		case MAC_MODE_DIFS:
			if (hasBackoff()) {
				...
				// 2007/10/08 濱田 浩平 キャリアセンス処理 START
				if (isCarrierIdle()) {
					sendPacket();
				} else {
					//retry();
					//2008/11/07 濱田 浩平 再バックオフを設定してidleへ START
					setBackoff();
					idle();
					//2008/11/07 濱田 浩平 再バックオフを設定してidleへ END
				}
				// 2007/10/08 濱田 浩平 キャリアセンス処理 END
		...
		case MAC_MODE_SBO:
			...
			// 2007/10/08 濱田 浩平 BO後のキャリアセンス処理 START
			...
			//
			//}
			// 2008/09/10 立山 崇之 BO後のキャリアセンス処理修正 START
			...
			// 2008/09/10 立山 崇之 BO後のキャリアセンス処理修正 END
			// 2007/10/08 濱田 浩平 BO後のキャリアセンス処理 END
			break;
		case MAC_MODE_SNAV_RTS:
		case MAC_MODE_SNAV:
			resetNav();
